use crate::{
    case::Case, distance::Far, flatcase::FlatCase, refraction::{OpRefraction, Refraction}, sca::Sca, surgeon::Surgeon, va::{FarVaSet, OpVa, Va}
};
...
async fn insert_case(case: Case) -> Result<Uuid, ServerFnError> {
    ...
    // we want to I think pass in a Vec of Cases, which means we really want to derive Queryable on
    // Case, and it also means this won't be single
